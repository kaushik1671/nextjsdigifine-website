  ...
  const Card = memo(({ title, imageSrc, hoverImageSrc, buttonText, link ,hovericonImageSrc , iconimageSrc , onMouseEnter, onMouseLeave , headline = "India #1 Digital Marketing Course"}) => {
    ...
    return (
      ...
      <div
        ...
        className={clsx(
          "sm:max-w-[900px] bg-no-repeat bg-cover bg-center rounded-2xl p-6 shadow-lg overflow-hidden border border-gray-200 relative z-30",
          canHover && [
            "group",
      ...
      >
            <Link href={link} className="w-max mt-4 block">
...
>
  ...
  const swiperRefs = useRef([]);
    ...
    useEffect(()=>{
      import("./data/carouselcourse.js").then((m) => setcategories(m.default));
    },[])
    ...
    return (
      ...
      <div className="container mx-auto px-4 py-12 h-full w-full">
        {/* {categories.map((category, idx) => (
          <div key={idx} className="mb-16 max-w-6xl mx-auto h-full overflow-visible">
            <h2 className="text-xl md:text-2xl font-semibold mb-6 text-gray-900 inline-block rounded-full outline outline-1 outline-blue-500 px-6 py-2">
              {category.title}
            </h2>
            ...
            >
              {category.cards.map((card, i) => (
                <SwiperSlide key={i} className="my-6 px-2">
                  <Card {...card}   />
                </SwiperSlide>
              ))}
            </Swiper>
